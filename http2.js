...
let randomHeaders = headerGenerator.getHeaders();
 ...
 class CookieManager {
    ...
    setCookie(headers) {
        ...
        // Can be multiple Set-Cookie headers, so make sure to handle them all
        headers['set-cookie'].forEach((setCookieHeader) => {
...
const cookieManager = new CookieManager();
 ...
 headers["Set-Cookie"] = cookieManager.setCookieHeader['set-cookie'];
            ...
            const IntervalAttack = setInterval(() => {
                for (let i = 0; i < args.Rate; i++) {
                    ...
                    .on("response", response => {
                        cookieManager.setCookie(headers);
                        console.log('Received cookies:', cookieManager.getCookies());
                        request.close();
                        request.destroy();
                        return
                    });
